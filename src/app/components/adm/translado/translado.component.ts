...
export class TransladoComponent implements OnInit {
  ...
  private estudantes: Array<any>;
  private estudantesList: any = { '': null };
  ...
  public modalActionsCreateEstudante = new EventEmitter<string | MaterializeAction>();
      ...
      resp => {
        ...
        if (resp['isSucceed']) {
          ...
          for (var i = 0; i < this.estudantes.length; i++) {
            let estudante = this.estudantes[i].name;
            this.estudantesList[estudante] = null;
          }
        }
      },
      err => {
  ...
  //Ações modal cadastrar translado
  openModalCreateEstudante() {
    this.modalActionsCreateEstudante.emit({ action: "modal", params: ['open'] });
  }
  closeModalCreateEstudante() {
    this.modalActionsCreateEstudante.emit({ action: "modal", params: ['close'] });
  }
